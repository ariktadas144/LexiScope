...
function App() {
  ...
  return (
    <div className="app-shell">
      <header className="topbar"><div className="brand">LexiScope</div><nav><a href="https://github.com/ariktadas144/LexiScope" target="_blank"><GitBranch size={15} /> Repository</a></nav></header>
      <section className="intro-row"><div><h1>See your language think.</h1><p className="subtitle">A transparent playground for Lexi, from source text to executable output.</p></div><div className="run-summary"><span className={hasError ? 'status-dot danger' : 'status-dot'}></span><span>{hasError ? 'Needs attention' : 'Pipeline healthy'}</span><span className="summary-divider" /><span>{result.tokens.filter((token) => token.kind !== 'eof').length} tokens</span></div></section>
      <main className="workspace-grid">
        <section className="panel editor-panel"><div className="panel-header"><div className="panel-title"><FileCode2 size={16} /><span>Source</span><span className="file-pill">main.lex</span></div><button className="run-button" type="button" onClick={() => setSource(source + '\n')}><CirclePlay size={15} /> Compile</button></div><div className="editor-toolbar"><label>Example <select value={selectedExample} onChange={(event) => { setSelectedExample(event.target.value); setSource(samplePrograms[event.target.value as keyof typeof samplePrograms]) }}>{Object.keys(samplePrograms).map((example) => <option key={example}>{example}</option>)}</select><ChevronDown size={13} /></label><span>Lexi / UTF-8</span></div><CodeEditor value={source} onChange={setSource} /><ErrorPanel error={result.error} /></section>
